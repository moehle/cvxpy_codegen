...
from cvxpy import Parameter, CallbackParam
import numpy as np
from cvxpy_codegen.expr_handler.explicit.sym_expr import SymConst, SymParam, SymExpr
...
class SymMatrix():
    ...
    def reshape_vec(self, m, n):
        if self.n > 1:
            raise Exception("Cannot reshape non-vectors.")

        Ap = [0]*(n+1)
        for p in range(len(self.Ai)):
            col = self.Ai[p] // m
            self.Ai[p] = self.Ai[p] % m
            #print()
            #print(self.Ai[p])
            #print(m)
            #print(col)
        ...
        return SymMatrix(Ap, self.Ai, self.Ax, m, n)
...
def denserowparam2sym(param):
    if len(param.shape) == 0:
        ...
        raise RuntimeError("Parameter cannot have dimenion greater than 2.")
    Ap = [0]
    Ai = []
    Ax = []
    for j in range(n):
