...
class TestOsqpIntf(tu.CodegenTestCase):
    ...
    def setUp(self):
        ...
        self.const_nn = self.const_nn.T.dot(self.const_nn)
        ...
        self.param_nn = cvx.Parameter((n, n), name='param_nn', value=self.const_nn)
    ...
    def test_nonpos(self):
        self._test_constrs([NonPos(1-self.var_n1)], solver='osqp')
        self._test_constrs([NonPos(self.var_n)], solver='osqp')
        self._test_constrs([NonPos(self.var_n1 + self.const_n1)], solver='osqp')
    ...
    def test_quad(self):
        ## TODO breaks cvxpy:
        ##self._test_expr(cvx.quad_form(self.var_n, self.param_nn), solver='osqp')
        self._test_expr(cvx.quad_form(self.var_n, self.const_nn), solver='osqp')
        self._test_expr(cvx.sum_squares(self.var_n), solver='osqp')
        self._test_expr(self.var +
                cvx.sum_squares(self.var_n + self.const_n), solver='osqp')
        self._test_expr(cvx.square(self.var_mn), solver='osqp')
        self._test_expr(cvx.square(self.var_n+self.param), solver='osqp')
        self._test_expr(cvx.square(self.var_n+self.param), solver='osqp')
        self._test_expr(cvx.quad_over_lin(
                self.var_mn, self.param+self.const), solver='osqp')
        self._test_expr(cvx.quad_over_lin(self.var_n+self.param,
                self.param+self.const), solver='osqp')
        #self._test_expr(cvx.pow_p(self.var_n+self.param,
        #        self.param+self.const), solver='osqp')
        self._test_expr(cvx.quad_over_lin(self.var_n+self.param,
                self.param+self.const), solver='osqp')
